...
def generate_gesture_chart(hands_data):
    ...
    try:
        ...
        plt.pie(percentages, labels=gestures, autopct='%1.1f%%', colors=colors[:len(gestures)])
        plt.title('Gesture Analysis Breakdown', fontsize=14, fontweight='bold')
        plt.axis('equal')
        
        # Save chart with optimized settings for PDF
        os.makedirs("data", exist_ok=True)
        plt.savefig("data/gesture_pie_chart.png", dpi=150, bbox_inches='tight', 
...
def generate_pacing_heatmap(pose_data):
    ...
    try:
        ...
        plt.title('Movement Heatmap - Presentation Pacing', fontsize=14, fontweight='bold')
        plt.grid(True, alpha=0.3)
        
        # Save chart with optimized settings for PDF
        os.makedirs("data", exist_ok=True)
        plt.savefig("data/pacing_heatmap.png", dpi=150, bbox_inches='tight',
                   facecolor='white', edgecolor='none', pad_inches=0.1)
        plt.close()
...
class ProfessionalPDF(FPDF):
    ...
    def footer(self):
        ...
        self.cell(0, 5, "Empowering confident communication through AI-powered analysis", ln=True, align='C')
...
def generate_pdf_report(face_data=None, hands_data=None, pose_data=None):
    ...
    try:
        ...
        if hands_data and not hands_data.get('error'):
            ...
            if 'gesture_percentages' in hands_data:
                ...
                for gesture, percentage in hands_data['gesture_percentages'].items():
                    ...
                    pdf.set_text_color(0, 0, 0)
                    pdf.cell(0, 6, f"  - {gesture.replace('_', ' ').title()}: {percentage:.1f}%", ln=True)
        ...
        # ===== VISUAL CHARTS PAGE =====
        if (hands_data and not hands_data.get('error')) or (pose_data and not pose_data.get('error')):
            pdf.add_page()
            ...
            # Gesture chart
            if hands_data and not hands_data.get('error') and os.path.exists("data/gesture_pie_chart.png"):
                try:
                    ...
                    pdf.image("data/gesture_pie_chart.png", x=x_position, y=y_position, w=image_width)
            ...
            # Movement heatmap
            if pose_data and not pose_data.get('error') and os.path.exists("data/pacing_heatmap.png"):
                try:
                    ...
                    # Add image with proper positioning
                    pdf.image("data/pacing_heatmap.png", x=x_position, y=y_position, w=image_width)
        ...
        # Add specific tasks based on analyses performed
        if "Eye Contact Analysis" in analyses_performed:
            week1_tasks.append("Practice eye contact exercises in front of a mirror")
        if "Body Posture Analysis" in analyses_performed:
            week1_tasks.append("Work on basic posture and breathing techniques")
        ...
        # Add specific tasks based on analyses performed
        if "Hand Gesture Analysis" in analyses_performed:
            week2_tasks.append("Practice specific gestures for different types of content")
        if "Body Posture Analysis" in analyses_performed:
        ...
        pdf.set_font("Arial", "", 10)
        pdf.set_text_color(0, 0, 0)
        pdf.multi_cell(0, 6, "Congratulations on taking the first step toward improving your presentation skills! Remember that effective public speaking is a skill that develops with practice and feedback.")
        pdf.ln(8)
        
        pdf.multi_cell(0, 6, "Continue using OratoCoach for regular practice sessions and track your progress over time. Each analysis will help you identify new areas for improvement and celebrate your successes.")
        pdf.ln(8)
        ...
        pdf.set_font("Arial", "", 9)
        pdf.set_text_color(0, 0, 0)
        pdf.cell(0, 6, "Continue practicing with OratoCoach and watch your confidence grow!", ln=True)
        
        # Save PDF
        os.makedirs("data", exist_ok=True)
        pdf.output("data/presentation_summary.pdf")
        
        print("✅ Professional PDF report generated using standardized template: data/presentation_summary.pdf")
        return True
