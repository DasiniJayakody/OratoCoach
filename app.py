...
from report_functions import (
    generate_pdf_report,
    generate_gesture_chart,
    generate_pacing_heatmap,
)
...
# Analysis status
analysis_status = {"is_running": False, "current_step": "", "progress": 0}
...
def print_run_instructions():
    ...
    print("")
    print(
        "3) Ensure 'data' directory exists (app creates it automatically if missing)."
    )
    print("")
    ...
    print("Notes:")
    print(
        " - If you use Docker or a different entrypoint, run the script with appropriate environment variables."
    )
...
def run_complete_analysis(selected_analyses):
    ...
    try:
        ...
        if "face" in selected_analyses:
            current_step += 1
            analysis_status["current_step"] = (
                "Analyzing eye contact and facial expressions..."
            )
        ...
        # Generate charts
        if analysis_results["hands_data"] and not analysis_results["hands_data"].get(
            "error"
        ):
            generate_gesture_chart(analysis_results["hands_data"])
        if analysis_results["pose_data"] and not analysis_results["pose_data"].get(
            "error"
        ):
...
def start_analysis():
    ...
    if analysis_status["is_running"]:
        if request.method == "POST":
            return jsonify(
                {"status": "error", "message": "Analysis already in progress"}
            ), 409
    ...
    if selected_analyses is None and request.form:
        ...
        form_list = request.form.getlist("analyses") or request.form.getlist(
            "analyses[]"
        )
        if form_list:
            ...
            if picked:
                ...
                if form_val:
                    try:
                        ...
                        selected_analyses = [
                            s.strip() for s in form_val.split(",") if s.strip()
                        ]

    # 3) Query string: /start_analysis?analyses=face,hands  or analyses[]=face
    if selected_analyses is None:
        args_list = request.args.getlist("analyses") or request.args.getlist(
            "analyses[]"
        )
        if args_list:
...
def results():
    ...
    if not analysis_results["analysis_completed"]:
        return render_template(
            "analysis.html", message="Please complete the analysis first."
        )
    ...
    results_data = {
        ...
        "has_face_data": analysis_results["face_data"]
        and not analysis_results["face_data"].get("error"),
        "has_hands_data": analysis_results["hands_data"]
        and not analysis_results["hands_data"].get("error"),
        "has_pose_data": analysis_results["pose_data"]
        and not analysis_results["pose_data"].get("error"),
    }
...
def generate_pdf():
    ...
    has_data_files = (
        os.path.exists("data/face_analysis_stats.json")
        or os.path.exists("data/hands_analysis_stats.json")
        or os.path.exists("data/pose_analysis_stats.json")
    )
    ...
    if not analysis_results["analysis_completed"] and not has_data_files:
        return jsonify(
            {
                "status": "error",
                "message": "Please complete the analysis first or ensure data files exist",
            }
        )
    ...
    try:
        ...
        print(
            f"📊 Data loaded - Face: {face_data is not None}, Hands: {hands_data is not None}, Pose: {pose_data is not None}"
        )

        # Generate PDF report
        success = generate_pdf_report(
            face_data=face_data, hands_data=hands_data, pose_data=pose_data
        )
        ...
        if success:
            ...
            with open("data/pdf_generated.txt", "w") as f:
                f.write(
                    f"PDF generated at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                )

            print("✅ PDF generated successfully in Flask app")
            return jsonify(
                {"status": "success", "message": "PDF generated successfully"}
            )
        else:
        ...
        traceback.print_exc()
        return jsonify(
            {"status": "error", "message": f"Error generating PDF: {str(e)}"}
        )
...
def download_pdf():
    ...
    if not os.path.exists(pdf_path):
        return jsonify(
            {"status": "error", "message": "PDF not found. Please generate it first."}
        )

    try:
        return send_file(
            pdf_path, as_attachment=True, download_name="presentation_summary.pdf"
        )
    except Exception as e:
        return jsonify(
            {"status": "error", "message": f"Error downloading PDF: {str(e)}"}
        )
...
def fix_analysis_status():
    ...
    if has_face_data or has_hands_data or has_pose_data:
        ...
        return jsonify({"status": "error", "message": "No analysis data files found"})
...
def check_pdf_exists():
    ...
    if os.path.exists(pdf_path):
        ...
        if file_size > 0:
            return jsonify({"exists": True, "size": file_size, "status": "ready"})
        else:
            return jsonify({"exists": True, "size": 0, "status": "empty"})
    else:
        return jsonify({"exists": False, "size": 0, "status": "not_found"})
...
def force_generate_pdf():
    ...
    try:
        ...
        if success and os.path.exists("data/presentation_summary.pdf"):
            ...
            return jsonify({"status": "error", "message": "PDF generation failed"})
